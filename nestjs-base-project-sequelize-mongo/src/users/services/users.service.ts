...
export class UsersService {
  ...
  async createNSQL(user: CreateUserDto): Promise<string> {
    try {
      const result: UserDTO = await this.userDao.createNSQL(user);
      return result.id;
    } catch (error) {
      throw new ErrorException(
        UserExceptions['7002'].code,
  ...
  async findAllNSQL(): Promise<UserDTO[]> {
    try {
      return await this.userDao.findAllNSQL();
    } catch (error) {
      throw new ErrorException(
        UserExceptions['7000'].code,
        UserExceptions['7000'].error,
